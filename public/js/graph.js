...
var nodes = [{id:0,reflexive:false},{id:1,reflexive:true},{id:2,reflexive:false}]
	lastNodeId=2,links=[{source:nodes[0],target:nodes[1],left:false,right:true},
	{source:nodes[1],target:nodes[2],left:false,right:true}];

var force = d3.layout.force().nodes(nodes)
	.links(links)
	.size([width,height])
	.linkDistance(150).charge(-500)
	.on('tick',tick);
...
function tick() {
	path.attr('d',function(d) {
		var deltaX = d.target.x - d.source.x,
			...
			normX = deltaX/dist,normY=deltaY/dist,
...
function restart() {
	...
	circle.selectAll('circle')
		.style('fill',function(d) {
			return(d === selected_node) ? d3.rgb(colors(d.id)).brighter().toString() : 
			colors(d.id);
		})
